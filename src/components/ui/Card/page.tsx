...
export default function Card({id, isNew, title, src, chapters = [], isPriority=false} : CardInterface){
    return (
    <div className="p-1.5 group hover:bg-card duration-300 rounded-md">
        <Link href={`/komik/${id}`} title={title}>
            ...
            <div className="mt-2">
                <p className="font-medium text-sm text-foreground/70 duration-400 truncate">{title}</p>
        ...
        <div className="flex flex-col gap-1 mt-2">
            {chapters.map(chapter => (
                <Link href={"chapters/"+chapter.id} className="flex justify-between items-center hover:bg-main/60 duration-300 group-hover:bg-main bg-second text-foreground/80 rounded-lg w-full py-2 text-sm px-2">
                    <span className="text-xs md:text-sm flex items-center gap-2"><i className="uil uil-link"></i> Chapter {chapter.chapterNumber}</span>
